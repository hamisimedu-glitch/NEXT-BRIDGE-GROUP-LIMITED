...
function App() {
  ...
  useEffect(() => {
    ...
    const { data: { subscription } } = onAuthChange(async (session, _user) => {
      (async () => {
        if (session?.user) {
          const profile = await fetchProfile(session.user.id);
          if (profile) setAdminUser({ ...profile, email: session.user.email ?? '' });
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#f4f1eb] text-[#17232b]">
      ...
      <main>
        ...
        {view === 'projects' && <ProjectsPage navigate={navigate} />}
        {view === 'units' && <UnitsPage navigate={navigate} setSelectedUnit={setSelectedUnit} />}
...
function ProjectsPage({ navigate }: { navigate: (view: View) => void }) { return <PageFrame eyebrow="The collection" title={<>Places to put down<br /><em>your roots.</em></>} intro="Explore the NBG portfolio. Each development will carry its own story, with verified information published as it becomes available."><div className="mt-16 grid gap-8 lg:grid-cols-2"><div className="group relative min-h-[520px] overflow-hidden"><img src={images.exterior} alt="Next Bridge Residences concept image" className="absolute inset-0 h-full w-full object-cover transition duration-700 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-[#17232b] to-transparent" /><div className="absolute bottom-8 left-8 text-white"><p className="eyebrow text-[#9edfeb]">Demo development · replace with verified details</p><h2 className="mt-3 font-serif text-5xl">Next Bridge<br /><em>Residences</em></h2><p className="mt-5 flex items-center gap-2 text-xs"><MapPin size={14} className="text-[#20afd1]" /> Nyali, Mombasa</p></div></div><div className="flex flex-col justify-between border-t border-[#c9c5bd] pt-7"><div><div className="flex items-center justify-between"><p className="eyebrow text-[#20afd1]">Now presenting</p><MoveUpRight size={19} className="text-[#20afd1]" /></div><p className="mt-8 max-w-md text-2xl leading-snug">A contemporary coastal address, currently in preparation.</p><p className="mt-6 max-w-md text-sm leading-6 text-slate-600">Project information, apartment types, pricing and construction milestones will be published here once verified by NBG.</p></div><div><div className="mt-10 grid grid-cols-2 gap-y-6 border-y border-[#c9c5bd] py-6">{[['Location', 'Nyali, Mombasa'], ['Status', 'Information pending'], ['Price', 'To be confirmed'], ['Availability', 'Coming soon']].map(([a, b]) => <div key={a}><p className="eyebrow text-slate-500">{a}</p><p className="mt-2 text-sm">{b}</p></div>)}</div><button onClick={() => navigate('units')} className="link-arrow mt-8">Explore availability <ArrowRight size={16} /></button></div></div></div></PageFrame> }
