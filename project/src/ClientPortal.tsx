...
function ClientPortal({ user, onSignOut, navigate }: { user: AdminUser; onSignOut: () => void; navigate: (view: View) => void }) {
  ...
  const saveProfile = async (values: { full_name: string; phone: string; preferred_location: string; investment_budget: string; notes: string }) => {
    ...
    const { data, error } = await supabase.from('profiles').upsert(payload, { onConflict: 'id' }).select('id, full_name, phone, preferred_location, investment_budget, notes').single();
    if (error) throw new Error(error.message || 'Profile could not be saved.');
    if (data) setProfile(data as { id: string; full_name: string; phone: string; preferred_location: string; investment_budget: string; notes: string });
  };
...
function OverviewContent({ user, projects, updates, units, investments, stages, payments, notifications, setSection }: { user: AdminUser; projects: Project[]; updates: ConstructionUpdate[]; units: ProjectUnit[]; investments: Investment[]; stages: ReservationStage[]; payments: ClientPayment[]; notifications: ClientNotification[]; setSection: (section: PortalSection) => void }) {
